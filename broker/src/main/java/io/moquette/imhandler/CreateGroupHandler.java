...
public class CreateGroupHandler extends GroupHandler<WFCMessage.CreateGroupRequest> {
    ...
    public ErrorCode action(ByteBuf ackPayload, String clientID, String fromUser, ProtoConstants.RequestSourceType requestSourceType, WFCMessage.CreateGroupRequest request, Qos1PublishHandler.IMCallback callback) {
        ...
        AppUserInfo appUserInfo = m_messagesStore.getAppUserInfo(fromUser);
        LOG.info("00000000 appUserInfo createGroupEnable: {}", appUserInfo.getCreateGroupEnable());
        if (!appUserInfo.getCreateGroupEnable()) {
            LOG.info("00000000 return ERROR_CODE_USER_CREATE_GROUP_DISABLED");
            return ErrorCode.ERROR_CODE_USER_CREATE_GROUP_DISABLED;
        }
