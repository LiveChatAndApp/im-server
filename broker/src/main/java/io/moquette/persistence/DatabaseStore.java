...
import com.xiaoleilu.loServer.action.enums.UserSettingScopeEnums;
import com.xiaoleilu.loServer.model.FriendData;
import com.xiaoleilu.loServer.sender.GroupDissolveMqSender;
import com.xiaoleilu.loServer.sender.GroupMemberSyncMqSender;
    ...
    void persistMessage(final WFCMessage.Message message, boolean update) {
        ...
        mScheduler.execute(()-> {
            ...
            try {
                ...
                LOG.info(">>>>>>>>>Upload Video: {}", message.getContent());
                ...
                try {
                    ...
                    msgMap.put("mid", message.getMessageId());
                    msgMap.put("from", message.getFromUser());
                    msgMap.put("to", message.getToUser());
                    msgMap.put("type", message.getConversation().getType());
                    msgMap.put("target", message.getConversation().getTarget());
                    msgMap.put("line", message.getConversation().getLine());

                    JSONObject json = new JSONObject();
                    json.put("searchableContent", message.getContent().getSearchableContent());
                    json.put("content", message.getContent().getContent());
                    json.put("extra", message.getContent().getExtra());
                    if (message.getContent().getData() != null && message.getContent().getData().size() > 0) {
                        json.put("data", Base64.getEncoder().encode(message.getContent().getData().toByteArray()));
                    }
                    if (message.getContent().getMediaType() > 0) {
                        json.put("mediaType", message.getContent().getMediaType());
                    }
                    if (!StringUtil.isNullOrEmpty(message.getContent().getRemoteMediaUrl())) {
                        json.put("remoteMediaUrl", message.getContent().getRemoteMediaUrl());
                    }

//                    String recalledContent = json.toJSONString();
                    msgMap.put("message", json);
                    msgMap.put("serverTimestamp", message.getServerTimestamp());
                    msgMap.put("contentType", message.getContent().getType());

                    UserMessageSendMqSender.send(msgMap);
                    LOG.info("Paige message mid: {}, extra: {}", message.getMessageId(), message.getContent().getExtra());
                } catch (Exception e) {
                    LOG.error("{} UserMessageSendMqSender error: {}", APIPath.Msg_Send, message, e);
                }
            } catch (SQLException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
    ...
    void persistGroupInfo(final WFCMessage.GroupInfo groupInfo) {
        ...
        try {
            ...
            long updateDt = groupInfo.getUpdateDt() == 0 ? System.currentTimeMillis() : groupInfo.getUpdateDt();
            long memberUpdateDt = groupInfo.getMemberUpdateDt() == 0 ? System.currentTimeMillis() : groupInfo.getUpdateDt();
            int index = 1;
            ...
            statement.setInt(index++, groupInfo.getHistoryMessage());
            statement.setLong(index++, memberUpdateDt);
            ...
            statement.setLong(index++, memberUpdateDt);
            int count = statement.executeUpdate();
            LOG.info("Update 群组 rows {}", count);
            sendGroupSyncMq(groupInfo, updateDt, memberUpdateDt);
        } catch (SQLException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
            Utility.printExecption(LOG, e, RDBS_Exception);
    ...
    private void sendGroupSyncMq(final WFCMessage.GroupInfo groupInfo, long updateDt, long memberUpdateDt) {
        try {
            Map<String, Object> msgMap = new HashMap<>();
            msgMap.put("gid", groupInfo.getTargetId());
            msgMap.put("name", groupInfo.getName());
            msgMap.put("portrait", groupInfo.getPortrait());
            msgMap.put("owner", groupInfo.getOwner());
            msgMap.put("type", groupInfo.getType());
            msgMap.put("extra", groupInfo.getExtra());
            msgMap.put("dt", updateDt);
            msgMap.put("memberCount", groupInfo.getMemberCount());
            msgMap.put("mute", groupInfo.getMute());
            msgMap.put("joinType", groupInfo.getJoinType());
            msgMap.put("privateChat", groupInfo.getPrivateChat());
            msgMap.put("searchable", groupInfo.getSearchable());
            msgMap.put("historyMessage", groupInfo.getHistoryMessage());
            msgMap.put("memberDt", memberUpdateDt);
            GroupSyncMqSender.send(msgMap);
        } catch (Exception e) {
            LOG.error("sendGroupSyncMq GroupSynMqSender error: {}", groupInfo, e);
        }
    }
    ...
    void persistGroupMember(final String groupId, final List<WFCMessage.GroupMember> memberList, boolean updateCreateTime) {
            ...
            try {
                ...
                for (WFCMessage.GroupMember member : memberList) {
                    ...
                    // 预设新增群至最爱群
//                    WFCMessage.UserSettingEntry settingEntry = WFCMessage.UserSettingEntry.newBuilder()
//                        .setScope(UserSettingScopeEnums.FAVORITE_GROUP.getValue())
//                        .setKey(groupId)
//                        .setValue("1")
//                        .setUpdateDt(dt).build();
//                    persistUserSetting(member.getMemberId(), settingEntry);
//                    LOG.info("已将{}加到{}的最爱群", groupId, member.getMemberId());

                    Map<String, Object> memeberMap = new HashMap<>();
                    memeberMap.put("gid", groupId);
                    memeberMap.put("mid", member.getMemberId());
                    memeberMap.put("alias", member.getAlias());
                    memeberMap.put("type", member.getType());
                    memeberMap.put("dt", dt);
                    memeberMap.put("extra", member.getExtra());
                    members.add(memeberMap);
                }
            } catch (SQLException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
        ...
        sendGroupMemberSyncMq(1, members);
    }
    private void sendGroupMemberSyncMq(Integer behavior, List<Map<String, Object>> members) {
        try {
            if (members.size() > 0) {
                Map<String, Object> map = new HashMap<>();
                map.put("behavior", behavior);
                map.put("members", members);
                GroupMemberSyncMqSender.send(map);
            }
        } catch (Exception e) {
            LOG.error("sendGroupMemberSyncMq error members: {}", members, e);
        }
    }
    ...
    int removeGroupMember(String groupId, List<String> groupMembers) {
        ...
        try {
            ...
            // 移除最爱群
            groupMembers.parallelStream().forEach(groupMember -> {
                WFCMessage.UserSettingEntry settingEntry = WFCMessage.UserSettingEntry.newBuilder()
                    .setScope(UserSettingScopeEnums.FAVORITE_GROUP.getValue())
                    .setKey(groupId)
                    .setValue("0")
                    .setUpdateDt(current).build();
                persistUserSetting(groupMember, settingEntry);
                LOG.info("已将{}移除{}的最爱群", groupId, groupMember);
            });

            sendGroupMemberSyncMq(2, members);
        } catch (SQLException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
            Utility.printExecption(LOG, e, RDBS_Exception);
...
//                try {
//                    if(request.getBlacked() == 1 ){
//                        // TODO: 新增黑名单
//                    }else{
//                        Map<String, Object> msgMap = new HashMap<>();
//                        msgMap.put("selfUid", request.getUserId());
//                        msgMap.put("targetUid", request.getFriendUid());
//                        msgMap.put("addTime", request.getTimestamp());
//                        msgMap.put("isAdd", request.getState() == 0);
//                        UserFriendSyncMqSender.send(msgMap);
//                    }
//                } catch (Exception e) {
//                    LOG.error("[IM][RabbitMQ] UserMessageSendMqSender error: {}", request, e);
//                }
    ...
    private void sendGroupDissolveMq(String groupId) {
        try {
            Map<String, Object> msgMap = new HashMap<>();
            msgMap.put("gid", groupId);
            GroupDissolveMqSender.send(msgMap);
        } catch (Exception e) {
            LOG.error("sendGroupDissolveMq GroupSyncMqSender error: {}", groupId, e);
        }
    }
