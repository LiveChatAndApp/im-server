...
import com.xiaoleilu.loServer.sender.GroupDissolveMqSender;
import com.xiaoleilu.loServer.sender.GroupSyncMqSender;
import com.xiaoleilu.loServer.sender.UserFriendSyncMqSender;
import com.xiaoleilu.loServer.sender.UserMessageSendMqSender;
...
import static win.liyufan.im.UserSettingScope.kUserSettingFavGroup;
...
public class MemoryMessagesStore implements IMessagesStore {
    ...
    static final String MSG_SEND_TIME = "msg_send_time";
    static final String MSG_LIMIT = "msg_limit";
    ...
    private final Map<Topic, StoredMessage> m_retainedStore = new HashMap<>();

    private final Server m_Server;

    private final Map<String, TreeMap<Long, Long>> userMessages = new HashMap<>();

    private final Map<String, TreeMap<Long, Long>> chatroomMessages = new HashMap<>();
    ...
    private final ConcurrentHashMap<String, Long> userMaxPullSeq = new ConcurrentHashMap<>();

    private SensitiveFilter mSensitiveFilter;
    private volatile long lastUpdateSensitiveTime = 0;

    private final ConcurrentHashMap<String, Boolean> userGlobalSlientMap = new ConcurrentHashMap<>();
    private final ConcurrentHashMap<String, Boolean> userVoipSlientMap = new ConcurrentHashMap<>();
    private final ConcurrentHashMap<String, Boolean> userPushHiddenDetail = new ConcurrentHashMap<>();
    private final ConcurrentHashMap<String, Boolean> userConvSlientMap = new ConcurrentHashMap<>();
    ...
    private long mFriendRejectDuration = 30L * 24 * 60 * 60 * 1000;
    ...
    private final List<Integer> mForbiddenClientSendTypes = new ArrayList<>();
    private final List<Integer> mBlackListExceptionTypes = new ArrayList<>();
    private final List<Integer> mGroupMuteExceptionTypes = new ArrayList<>();
    private final List<Integer> mGlobalMuteExceptionTypes = new ArrayList<>();
    ...
    private String mUserInfoConnection;

    private final Set<Integer> mUserHideProperties = new HashSet<>();

    private IConfig iconfig = null;

    MemoryMessagesStore(Server server, DatabaseStore databaseStore, IConfig iconfig) {
        m_Server = server;
        ...
        try {
            ...
            LOG.error("USER_INFO_UPDATE_CALLBACK error: {}", e);
        }
        ...
        try {
            mUserInfoConnection = server.getConfig().getProperty(USER_INFO_CONNECTION);
        } catch (Exception e) {
...
//        }
    ...
    public WFCMessage.GroupInfo createGroup(String fromUser, WFCMessage.GroupInfo groupInfo, List<WFCMessage.GroupMember> memberList, String memberExtra, boolean isAdmin) {
        ...
        updatedMemberList.parallelStream().forEach(member -> {
            updateUserSettings(member.getMemberId(), WFCMessage.ModifyUserSettingReq.newBuilder().setScope(kUserSettingFavGroup).setKey(finalGroupId).setValue("1").build(), "");
            LOG.info("已将{}加到{}的最爱群", finalGroupId, member.getMemberId());
        });
    ...
    private void callbackUserInfoEvent(WFCMessage.User user) {
        if (!StringUtil.isNullOrEmpty(mUserInfoUpdateCallback)) {
            m_Server.getCallbackScheduler().execute(() -> {
                try {
                    String httpFilePath = m_Server.getConfig().getProperty(BrokerConstants.HTTP_FILE_PATH);
    ...
    @Override
    public ErrorCodeView addGroupMembers(String operator, boolean isAdmin, String groupId, List<WFCMessage.GroupMember> memberList, String extra) {
        ErrorCodeView errorCodeView = new ErrorCodeView();
        HazelcastInstance hzInstance = m_Server.getHazelcastInstance();
        IMap<String, WFCMessage.GroupInfo> mIMap = hzInstance.getMap(GROUPS_MAP);

        WFCMessage.GroupInfo groupInfo = mIMap.get(groupId);
        errorCodeView.setObject(groupInfo);
        if (groupInfo == null) {
            errorCodeView.setErrorCode(ErrorCode.ERROR_CODE_NOT_EXIST);
        ...
        if (!isAdmin) {
            ...
            if (gm != null) {
                if (gm.getType() == GroupMemberType_Removed && !(memberList.size() == 1 && operator.equals(memberList.get(0).getMemberId()))) {
                    errorCodeView.setErrorCode(ErrorCode.ERROR_CODE_NOT_IN_GROUP);
                    return errorCodeView;
                } else if (gm.getType() == GroupMemberType_Silent) {
                    errorCodeView.setErrorCode(ErrorCode.ERROR_CODE_NOT_RIGHT);
                    return errorCodeView;
                }
        ...
        if (memberList.size() == 0) {
            if (!isAdmin) {
                errorCodeView.setErrorCode(ErrorCode.ERROR_CODE_ALREADY_IN_GROUP);
                return errorCodeView;
            } else {
                errorCodeView.setErrorCode(ErrorCode.ERROR_CODE_SUCCESS);
                return errorCodeView;
            }
        } else {
            if (existMemberIds.size() + newMemberCount > maxCount) {
                errorCodeView.setErrorCode(ErrorCode.ERROR_CODE_GROUP_EXCEED_MAX_MEMBER_COUNT);
                return errorCodeView;
            }
        }
        ...
        for (WFCMessage.GroupMember member : memberList) {
            if (member.getType() != GroupMemberType_Removed) {
                ...
                updateUserSettings(member.getMemberId(), WFCMessage.ModifyUserSettingReq.newBuilder().setScope(kUserSettingFavGroup).setKey(groupId).setValue("1").build(), "");
                LOG.info("已将{}加到{}的最爱群", groupId, member.getMemberId());
            }
        }
    ...
    public ErrorCode quitGroup(String operator, String groupId) {
        ...
        removeGroupMember(groupId, Collections.singletonList(operator));
        removeFavGroup(groupId, Collections.singletonList(operator));
        removeGroupUserSettings(groupId, Collections.singletonList(operator));

        callbackGroupMemberEvent(operator, groupId, Collections.singletonList(operator), ProtoConstants.GroupMemberUpdateEventType.Group_Member_Event_Leave, null);
        return ErrorCode.ERROR_CODE_SUCCESS;
    }
        ...
        if (!isAdmin) {
            ...
            if (oldInfo.getType() == ProtoConstants.GroupType.GroupType_Normal) {
                if (!operator.equals(oldInfo.getOwner())) {
                    WFCMessage.GroupMember gm = getGroupMember(groupId, operator);
                    boolean isManager = gm != null && gm.getType() == GroupMemberType_Manager;
    ...
    public ErrorCode modifyGroupMemberAlias(String operator, String groupId, String alias, String memberId, boolean isAdmin) {
        ...
        for (WFCMessage.GroupMember member : members) {
            if (member.getMemberId().equals(memberId)) {
                ...
                databaseStore.persistGroupMember(groupId, Collections.singletonList(member), false);
        ...
        callbackGroupMemberEvent(operator, groupId, Collections.singletonList(memberId), ProtoConstants.GroupMemberUpdateEventType.Group_Member_Event_Alias, alias);
        return ErrorCode.ERROR_CODE_SUCCESS;
    }
    ...
    public ErrorCode modifyGroupMemberExtra(String operator, String groupId, String extra, String memberId, boolean isAdmin) {
        ...
        callbackGroupMemberEvent(operator, groupId, Collections.singletonList(memberId), ProtoConstants.GroupMemberUpdateEventType.Group_Member_Event_Alias, extra);
        return ErrorCode.ERROR_CODE_SUCCESS;
    }
    ...
    public WFCMessage.ChatroomMemberInfo getChatroomMemberInfo(String chatroomId, final int maxMemberCount) {
        ...
        if (members != null) {
            ...
            if (members.size() <= maxMemberCount) {
                ...
                while (p.estimateSize() > 2L * maxMemberCount) {
                    p = p.trySplit();
                }
...
//            }
    ...
    public ErrorCode handleFriendRequest(String userId, WFCMessage.HandleFriendRequest request, WFCMessage.Message.Builder msgBuilder, long[] heads, boolean isAdmin) {
        ...
        if (isAdmin) {
            ...
            //新增好友RabbitMQ
            UserFriendSyncMqSender.sendAddFriend(friendData1.getUserId(), friendData1.getFriendUid(), friendData1.getTimestamp(), friendData1.getBlacked(), friendData1.getState());
    ...
    public ErrorCode blackUserRequest(String fromUser, String targetUserId, int state, long[] heads) {
        ...
        UserFriendSyncMqSender.sendAddFriend(fromUser, targetUserId, friendData.getTimestamp(), state, UserFriendSyncMqSender.BLOCK_FRIEND);
        return ErrorCode.ERROR_CODE_SUCCESS;
    }
    ...
    public ErrorCode handleJoinChatroom(String userId, String clientId, String chatroomId) {
        ...
        callbackChatroomMemberEvent(userId, chatroomId, Collections.singletonList(userId), Chatroom_Member_Event_Join);
    ...
    public ErrorCode handleQuitChatroom(String userId, String clientId, String chatroomId) {
        ...
        callbackChatroomMemberEvent(userId, chatroomId, Collections.singletonList(userId), Chatroom_Member_Event_Leave);
        return ErrorCode.ERROR_CODE_SUCCESS;
    }
    ...
    public boolean checkChatroomParticipantIdelTime(MemorySessionStore.Session session) {
        ...
        return mChatroomParticipantIdleTime <= 0 || System.currentTimeMillis() - session.getLastChatroomActiveTime() <= mChatroomParticipantIdleTime;
    }
    ...
    public ErrorCode deleteFriend(String userId, String friendUid, long[] head) {
        ...
        for (FriendData data :
            ...
            if (data.getFriendUid().equals(friendUid)) {
                ...
                // 移除好友
                UserFriendSyncMqSender.sendAddFriend(data.getUserId(), data.getFriendUid(), data.getTimestamp(), data.getBlacked(), data.getState());
    ...
    public boolean getUserGlobalSilent(String userId) {
        ...
        if (slient == null) {
            ...
            slient = entry != null && entry.getValue().equals("1");
    ...
    public boolean getUserPushHiddenDetail(String userId) {
        ...
        if (hidden == null) {
            ...
            hidden = entry != null && entry.getValue().equals("1");
    ...
    public boolean canSendMessageInChannel(String user, String channelId) {
        ...
        return channelMembers != null && channelMembers.contains(user);
    }
    ...
    public boolean checkUserInChannel(String user, String channelId) {
        ...
        if (!channelMembers.contains(user)) {
            IMap<String, WFCMessage.ChannelInfo> mIMap = m_Server.getHazelcastInstance().getMap(CHANNELS);
            WFCMessage.ChannelInfo info = mIMap.get(channelId);
            return info != null && info.getOwner().equals(user);
        }
        return true;
    }
    ...
    @Override
    public AppUserInfo getAppUserInfo(String userId) {
        AppUserInfo userInfo = null;
        if (!StringUtil.isNullOrEmpty(mUserInfoConnection)) {
            UserInfoRequest request = new UserInfoRequest();
            request.setUid(userId);
            try {
                String json = HttpUtils.httpJsonPost(mUserInfoConnection, GsonUtil.gson.toJson(request));
                LOG.info("getAppUserInfo json: {}", json);
                userInfo = new ObjectMapper().readValue(json, AppUserInfo.class);
                LOG.info("getAppUserInfo userInfo: {}", userInfo);
            } catch (Exception e) {
                e.printStackTrace();
                Utility.printExecption(LOG, e, EVENT_CALLBACK_Exception);
            }
    ...
    public void setMSGSendLimit(String userId) {
        HazelcastInstance hzInstance = m_Server.getHazelcastInstance();
        MultiMap<String, Date> requestMap = hzInstance.getMultiMap(MSG_SEND_TIME);
        requestMap.put(userId, new Date());
        LOG.info("[setMSGSendLimit]: {}", requestMap);
    }

    public Boolean isMSGSendLimit(String userId) {
        HazelcastInstance hzInstance = m_Server.getHazelcastInstance();
        MultiMap<String, Date> requestMap = hzInstance.getMultiMap(MSG_SEND_TIME);
        Map<String, Date> limitMap = hzInstance.getMap(MSG_LIMIT);
        Date limitDate = limitMap.get(userId);
        List<Date> dateList = requestMap.get(userId).stream().sorted(Collections.reverseOrder()).collect(Collectors.toList());

        int time = Integer.parseInt(iconfig.getProperty(BrokerConstants.MESSAGE_LIMIT_TIME));
        int second = Integer.parseInt(iconfig.getProperty(BrokerConstants.MESSAGE_LIMIT_SECOND));
        int secondBlock = Integer.parseInt(iconfig.getProperty(BrokerConstants.MESSAGE_LIMIT_SECOND_BLOCK));

        int count = 0;
        Date currentDate = new Date();
        LOG.info("[isMSGSendLimit] 讯息发送: {}", dateList);

        if (limitDate != null && limitDate.after(currentDate)) {
            LOG.info("[isMSGSendLimit] 限制讯息直到: {}", limitDate);
            return true;
        }
        ...
        for (Date date: dateList) {
            ...
            if (limit.after(currentDate)) {
                LOG.info("[isMSGSendLimit] limit: {}, currentDate: {}, count: {}", limit, currentDate, count);
                count ++;
            }else if (limit.before(currentDate)) {
                LOG.info("[isMSGSendLimit] 移除date:{}", date);
                requestMap.remove(userId, date);
            }

            if (count > time) {
                Calendar limitCalendar = new GregorianCalendar();
                limitCalendar.setTime(currentDate);
                limitCalendar.add(Calendar.SECOND, secondBlock);
                LOG.info("[isMSGSendLimit] 限制讯息直到: {}", limitCalendar.getTime());
                limitMap.put(userId, limitCalendar.getTime());
                isLimit = true;
            }
        }
