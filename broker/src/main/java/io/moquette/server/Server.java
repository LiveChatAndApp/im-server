...
public class Server {
    ...
    public static void start(String[] args) throws Exception {
        ...
        String mqHost = config.getProperty(BrokerConstants.SPRING_RABBITMQ_HOST);
        String mqPort = config.getProperty(BrokerConstants.SPRING_RABBITMQ_PORT);
        String mqUserName = config.getProperty(BrokerConstants.SPRING_RABBITMQ_USERNAME);
        String mqPassword = config.getProperty(BrokerConstants.SPRING_RABBITMQ_PASSWORD);

        LOG.info("Server Start >>> RabbitMq host: {}, port: {}, userName: {}, password: {}", mqHost, mqPort, mqUserName, mqPassword);

        RabbitMqConfig.connectionFactory(mqHost, mqPort, mqUserName, mqPassword);
    ...
    private boolean configureCluster(IConfig config) throws FileNotFoundException {
        ...
        if (hzConfigPath != null) {
            ...
            Boolean enableClusterByTcpIp = Boolean.valueOf(config.getProperty(BrokerConstants.HAZELCAST_TCPIP_ENABLE));
            hzconfig.getNetworkConfig().getJoin().getTcpIpConfig().setEnabled(enableClusterByTcpIp);
